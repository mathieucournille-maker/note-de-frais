...
async function buildPDF(note) {
  ...
  doc.text("Note de frais - Mathieu COURNILLE", ML, 16);

  // Date en format français JJ/MM/AAAA
  const dateFR = note.date.split("-").reverse().join("/");
  doc.setFontSize(10); doc.setFont("helvetica", "normal"); doc.setTextColor(200, 210, 230);
  doc.text(`Date : ${dateFR}`, ML, 27);

  // Motif
  if (note.motif) doc.text(`Motif : ${note.motif}`, ML, 36);

  // Trajet avec double flèche aller-retour ⇄
  doc.text(`Trajet : ${note.depart} \u21c4 ${note.destination}`, ML, 45);

  // Encart TOTAL À REMBOURSER (colonne droite)
  doc.setFont("helvetica", "bold"); doc.setFontSize(8); doc.setTextColor(150, 200, 190);
  doc.text("TOTAL À REMBOURSER", PW - MR, 22, { align: "right" });
  doc.setFontSize(20); doc.setTextColor(0, 201, 167);
  ...
  const lignes = [
    ...
    note.repas   > 0 && { label: "Restaurant / Frais de bouche", detail: "", montant: note.repas },
    note.cadeaux > 0 && { label: "Cadeaux clients",            detail: "", montant: note.cadeaux },
  ].filter(Boolean);
...
function NewNote({ onSaved, onCancel }) {
  ...
  const totalRepas  = parseFloat(repas   || 0);
  const totalCadx   = parseFloat(cadeaux || 0);
  ...
  const buildNote = () => ({
    ...
    repas: totalRepas, cadeaux: totalCadx, photos,
      ...
      <div style={S.body}>
        ...
        {step === 1 && <>
          ...
          <div style={S.card}>
            ...
            <input style={S.input} type="number" value={repas}
              onChange={e => setRepas(e.target.value)} placeholder="0.00" />
          </div>

          <div style={S.card}>
            <div style={S.secTitle}>🎁 Cadeaux clients</div>
            <label style={S.label}>Montant total (€)</label>
            <input style={S.input} type="number" value={cadeaux}
              onChange={e => setCadeaux(e.target.value)} placeholder="0.00" />
          </div>
        ...
        {step === 2 && <>
          <div style={S.card}>
            ...
            {totalRepas > 0 && (
              <div style={S.sumRow}>
                <span>🍽 Restaurant / Frais de bouche</span>
                <span style={{ fontWeight: 700, color: C.blue }}>{fmt(totalRepas)}</span>
              </div>
            )}
            {totalCadx > 0 && (
              <div style={S.sumRow}>
                <span>🎁 Cadeaux clients</span>
                <span style={{ fontWeight: 700, color: C.blue }}>{fmt(totalCadx)}</span>
              </div>
            )}
